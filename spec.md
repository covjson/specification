...
- A parameter object may have a member with the name `"categoryEncoding"` where the value is a non-empty array of category mapping objects of which each must have a `"category"` member and either a `"value"` or `"values"` member. The value of `"category"` must be equal to an `"id"` value of the `"categories"` array within the `"observedProperty"` member of the parameter object. There must be no duplicate `"category"` values within the `"categoryEncoding"` array. If given, the value of `"value"` must be an integer unique within all category mapping objects. If given, the value of `"values"` must be an array of `"value"` integers.
